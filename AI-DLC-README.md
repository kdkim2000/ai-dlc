...
**목표**: 개발·검증 완료 후 모든 산출물을 납품 패키지로 정리
...
(납품검수목록_*.md)             산출물 수집 + 필수 문서 누락 보고
         │
    ┌────┼─────────────────┐
    ▼    ▼                 ▼
  시스템개요서  사용자매뉴얼   개발자가이드
(모든 산출물 통합) (화면→UX)  (API+DB+소스구조)
    └────┴─────────────────┘
                │
                ▼
...
| 스킬 | 트리거 예시 | 산출물 |
|:---|:---|:---|
| `ai-dlc-delivery-checklist` | "납품 검수 목록 만들어줘", "납품 산출물 확인해줘" | 납품검수목록_*.md |
...
| 스킬명 | 역할 |
|:---|:---|
| `ai-dlc-delivery-checklist` | 단계별 산출물 수집 + 필수·권장 항목 검수 |
| `ai-dlc-system-overview` | 모든 산출물 통합 → 시스템 개요서 |
| `ai-dlc-user-manual` | 화면정의서·유즈케이스 → 사용자 매뉴얼 |
| `ai-dlc-dev-guide` | API·DB·소스코드 → 개발자 가이드 |
| `ai-dlc-delivery-package` | 납품 패키지 인덱스 + Word 변환 안내 |
